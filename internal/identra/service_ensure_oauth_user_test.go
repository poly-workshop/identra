...
func (m *mockUserStore) Create(ctx context.Context, user *UserModel) error {
...
func (m *mockUserStore) GetByID(ctx context.Context, id string) (*UserModel, error) {
...
func (m *mockUserStore) GetByEmail(ctx context.Context, email string) (*UserModel, error) {
...
func (m *mockUserStore) Update(ctx context.Context, user *UserModel) error {
...
func (m *mockUserStore) List(ctx context.Context, offset, limit int) ([]*UserModel, error) {
	result := make([]*UserModel, 0, len(m.users))
	for _, user := range m.users {
...
type mockExternalIdentityStore struct {
	identities     []*ExternalIdentityModel
...
func (m *mockExternalIdentityStore) Create(_ context.Context, identity *ExternalIdentityModel) error {
	if m.forceCreateErr != nil {
		return m.forceCreateErr
	}
	if identity.ID == "" {
...
func (m *mockExternalIdentityStore) GetByUserID(
	...
	var result []*ExternalIdentityModel
	for _, id := range m.identities {
		if id.UserID == userID {
...
func TestEnsureOAuthUser_ExistingUserByProviderID(t *testing.T) {
	...
	_ = extStore.Create(context.Background(), &ExternalIdentityModel{
		ID:             "eid1",
...
func TestEnsureOAuthUser_IdentityCreateFailure_OrphanedUserDeleted_WithEmail(t *testing.T) {
	store := newMockUserStore()
	extStore := newMockExternalIdentityStore()
	extStore.forceCreateErr = ErrAlreadyExists
	svc := &Service{userStore: store, externalIdentityStore: extStore}
